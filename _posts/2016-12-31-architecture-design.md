...
在进行软件架构设计的时候，其实大部分会考虑说如何进行开发了问题，如果是使用水平优先的这种方式，那么我们需要预先把所有的功能都进行定义，然后再逐步铺开进行开发，那如果为了说能够更好地利用交互，及时得到用户的反馈，那么我们会选择垂直优先的这种方式，垂直优先策略的说我们需要把垂直相关的这种系统功能跟架构支持先定义好。因此在常见的分层，架构之上，我们还会构建出对应的分区规则。分区构建在分层之内，其粒度比分层更小。一般一个具体的垂直功能只跟一个分层中具体的某一个分区有关系。垂直划分的模式更接近目前比较流行的微服务的概念，通过将开发团队和团队目标分割成为彼此较为独立的服务，便于快速迭代和敏捷开发。定义好分层跟分区试点进行迭代开发的关键。
...
![architecture](https://imgur.com/a/p8QqY)
